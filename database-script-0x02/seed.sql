...
SELECT gen_random_uuid(),
    ...
    u.user_id,
    5,
    'Amazing stay! Very clean and host was super friendly.'
FROM properties p
...
SELECT gen_random_uuid(),
    ...
    u.user_id,
    4,
    'Great stay, but WiFi could be faster.'
FROM properties p
...
SELECT gen_random_uuid(),
    ...
    u.user_id,
    3,
    'Booking got canceled, but the host communicated well.'
FROM properties p
...
FROM users u1,
    users u2
WHERE u1.email = 'fiona@example.com'
    AND u2.email = 'helen@example.com';
-- Helen replies
INSERT INTO messages (
        message_id,
        sender_id,
...
FROM users u1,
    users u2
WHERE u1.email = 'helen@example.com'
    AND u2.email = 'fiona@example.com';
...
FROM users u1,
    users u2
WHERE u1.email = 'george@example.com'
    AND u2.email = 'edward@example.com';
-- Edward replies
INSERT INTO messages (
        message_id,
        sender_id,
...
FROM users u1,
    users u2
WHERE u1.email = 'edward@example.com'
    AND u2.email = 'george@example.com';